...
void main()
{
    ...
    vec3 col_sunlight = cosinePalette((fs_FBM/1.5) * fs_Worley);
    vec4 col_step2 = vec4(mix(col_sunlight * 1.7, bg_Col, t), 1.0);

    vec4 col_final = vec4(mix(col_step, col_step2, 0.2));
    //vec4 col_final = vec4(mix(vec3(col_step), vec3(col_step2), 0.2), 1.0);
    
    //vec3 col_step = vec3(mix(bg_Col, sand_Col, vec3(0.25 * (fs_FBM + 1.0))));

    //out_Col = col_step + col_step2;
    //out_Col = col_step2;
    out_Col = col_final;
